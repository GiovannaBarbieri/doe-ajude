...
$ctrl = new DoadorCTRL();
$ctrl_pes = new PessoaCTRL();

$enderecoUser = $ctrl->ConsultarEnderecoPessoa();

$parceiro = $ctrl->retornarParceiro();
...
if (isset($_POST['btnSalvar'])) {
    ...
    if ($foto['type'] != 'image/jpeg') {
        ...
        if ($endereco == 1) {
            $vo->setIdParceiro($_POST['parceiro']);
        ...
        $ret = $ctrl->InserirDoacaoParceiro($vo);
    }
}
...
?>
                            <h2>Quero Doar</h2>   
                            <h5>Realize a sua doação aqui e acompanhe o status em <a href="consultar_doacoes_parceiro.php">consultar doações.</a></h5>
                        </div>
                    </div>
                    <!-- /. ROW  -->
                    <hr />
                    <form method="post" action="quero_doar_parceiro.php" enctype="multipart/form-data">
                        <input type="hidden" value="<?= $parceiro[0]['id_parceiro'] ?>" name="parceiro">
                            <div class="col-md-6">
